...
import argparse, json, math, os, random, shutil, time, zipfile
...
def find_pairs(data_root: Path) -> Tuple[List[Tuple[Path, Path, str]], List[Path]]:
    paths = [p for p in list_images(data_root) if "__macosx" not in str(p).lower()]
    ...
    if not pairs:
        raise RuntimeError(
            "No train pairs found. Expected names like rain-1.png / rain_clean-1.png "
            "and snow-1.png / snow_clean-1.png somewhere under the extracted dataset."
        )
...
class RestorationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i, j, h, w = transforms.RandomCrop.get_params(deg, (self.crop, self.crop))
...
class MDTA(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias=False):
        ...
        self.qkv_d = nn.Conv2d(dim*3, dim*3, 3, padding=1, groups=dim*3, bias=bias)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_factor=2.66, bias=False, ln_bias=True):
        super().__init__()
...
def make_encoder_level(dim, n_blocks, num_heads):
    return nn.Sequential(*[TransformerBlock(dim, num_heads) for _ in range(n_blocks)])
...
class Downsample(nn.Module):
    def __init__(self, in_c):
        ...
        self.ps   = nn.PixelUnshuffle(2)   # (B, in_c*2, H, W) → (B, in_c*8, H/2, W/2)
...
class PromptIR(nn.Module):
    def __init__(self, dim=64, num_blocks=(4,6,6,8), num_heads=(1,2,4,8),
                 ffn_factor=2.66, prompt_len=8):
        super().__init__()
        ...
        self.down1 = Downsample(dim)        # dim   → dim*2  (but PixelUnshuffle makes dim*8 → need fix)
        self.down2 = Downsample(dim*2)
        self.down3 = Downsample(dim*4)

        # fix channel math: Conv(in_c, in_c*2) + PixelUnshuffle(2) →
        #   in_c*2 * 4 = in_c*8  ... that's wrong for standard Restormer.
        # Use correct Restormer downsample: Conv(in_c, in_c*2, bias=False) only,
        # spatial done separately with stride.  We redefine cleanly:
        self.down1 = nn.Sequential(nn.Conv2d(dim,   dim*2, 4, stride=2, padding=1, bias=False))
        self.down2 = nn.Sequential(nn.Conv2d(dim*2, dim*4, 4, stride=2, padding=1, bias=False))
        self.down3 = nn.Sequential(nn.Conv2d(dim*4, dim*8, 4, stride=2, padding=1, bias=False))

        # Bottleneck
        self.bottleneck = make_encoder_level(dim*8, num_blocks[3], num_heads[3])

        # Decoder (with prompt blocks at each level)
        self.up3    = nn.Sequential(nn.Conv2d(dim*8, dim*4*4, 3, padding=1, bias=False), nn.PixelShuffle(2))
        self.skip3  = nn.Conv2d(dim*8, dim*4, 1, bias=False)
        self.dec3   = make_encoder_level(dim*4, num_blocks[2], num_heads[2])
        self.prompt3 = PromptBlock(dim*4, num_heads[2], prompt_len)  # channels = dim*4

        self.up2    = nn.Sequential(nn.Conv2d(dim*4, dim*2*4, 3, padding=1, bias=False), nn.PixelShuffle(2))
        self.skip2  = nn.Conv2d(dim*4, dim*2, 1, bias=False)
        self.dec2   = make_encoder_level(dim*2, num_blocks[1], num_heads[1])
        self.prompt2 = PromptBlock(dim*2, num_heads[1], prompt_len)  # channels = dim*2

        self.up1    = nn.Sequential(nn.Conv2d(dim*2, dim*4, 3, padding=1, bias=False), nn.PixelShuffle(2))
        self.skip1  = nn.Conv2d(dim*2, dim, 1, bias=False)
        self.dec1   = make_encoder_level(dim, num_blocks[0], num_heads[0])
        self.prompt1 = PromptBlock(dim, num_heads[0], prompt_len)    # channels = dim
    ...
    def forward(self, x):
        ...
        d3  = self.prompt3(self.dec3(self.skip3(torch.cat([self.up3(b),  e3], dim=1))))
        d2  = self.prompt2(self.dec2(self.skip2(torch.cat([self.up2(d3), e2], dim=1))))
        d1  = self.prompt1(self.dec1(self.skip1(torch.cat([self.up1(d2), e1], dim=1))))
...
def evaluate(model, loader, device, use_tta=False):
    ...
    for deg, clean, typs in loader:
        ...
        for i, t in enumerate(typs):
            ...
            if t == "rain": rain_psnr.append(v)
            else:           snow_psnr.append(v)
    r = float(np.mean(rain_psnr)) if rain_psnr else 0.
    s = float(np.mean(snow_psnr)) if snow_psnr else 0.
...
def run_stage(
    model, ema, train_loader, val_loader, device,
    epochs, lr, warmup, ema_decay,
    ckpt_dir: Path, stage_name: str,
    history: dict,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(f"[{stage_name}] Ep {epoch+1:4d}/{epochs}  "
              f"loss={avg_loss:.5f}  "
...
def _plot_one_stage_loss(ax, data, title, colour):
    ...
    ax.set_title(title, fontsize=13, fontweight="bold")
    ax.set_xlabel("Epoch"); ax.set_ylabel("Charbonnier Loss")
    ax.grid(alpha=0.3)
...
def plot_history(history: dict, out_dir: Path):
    ...
    if "stage1" in history:
        ...
        _plot_one_stage_psnr(ax, history["stage1"],
                             LABELS["stage1"], COLOURS["stage1"])
        plt.tight_layout()
    ...
    if "stage1" in history:
        ...
        _plot_one_stage_loss(ax, history["stage1"],
                             LABELS["stage1"], COLOURS["stage1"])
        plt.tight_layout()
    ...
    if "stage2" in history:
        fig, ax = plt.subplots(figsize=(8, 5))
        _plot_one_stage_psnr(ax, history["stage2"],
                             LABELS["stage2"], COLOURS["stage2"])
        plt.tight_layout()
    ...
    if "stage2" in history:
        fig, ax = plt.subplots(figsize=(8, 4))
        _plot_one_stage_loss(ax, history["stage2"],
                             LABELS["stage2"], COLOURS["stage2"])
        plt.tight_layout()
    ...
    if len(history) >= 1:
        ...
        for i, sname in enumerate(stage_names):
            ...
            ax.axvspan(x0, x1, alpha=0.25, color=bg, label=LABELS.get(sname, sname))
            # stage label at top
            mid = (x0 + x1) / 2
            ax.text(mid, ax.get_ylim()[1] if ax.get_ylim()[1] > 0 else 32,
        ...
        ax.set_title("Full Training — PSNR Across All Stages (Rain / Snow / Total)",
    ...
    if len(history) >= 1:
        ...
        for i, sname in enumerate(stage_names):
            ...
            mid = (x0 + x1) / 2
            ax.text(mid, max(all_loss) * 0.98,
                    LABELS.get(sname, sname).split("—")[0].strip(),
                    ha="center", va="top", fontsize=9,
                    color=COLOURS.get(sname, "#555"))

        ax.set_xlabel("Epoch (cumulative)"); ax.set_ylabel("Charbonnier Loss")
        ax.set_title("Full Training — Loss Across All Stages",
                     fontsize=13, fontweight="bold")
        ax.grid(alpha=0.3)
        plt.tight_layout()
...
def parse_args():
    ...
    p.add_argument("--data_root",  default="vis_hw4",
...
def main():
    ...
    if not args.infer_only:
        ...
        print("="*60)
        train_ds1 = RestorationDataset(train_pairs, crop=args.crop, augment="full")
        train_loader1 = DataLoader(train_ds1, batch_size=args.batch, shuffle=True,
                                   num_workers=args.workers, pin_memory=True,
                                   drop_last=True)
        s1_best, history = run_stage(
            model, ema, train_loader1, val_loader, device,
            epochs=args.s1_epochs, lr=args.s1_lr,
            warmup=args.s1_warmup, ema_decay=args.s1_ema,
            ckpt_dir=ckpt_dir, stage_name="stage1",
            history=history,
        )
        ...
        print("="*60)
        train_ds2 = RestorationDataset(train_pairs, crop=args.crop, augment="hflip")
        train_loader2 = DataLoader(train_ds2, batch_size=args.batch, shuffle=True,
                                   num_workers=args.workers, pin_memory=True,
                                   drop_last=True)
        s2_best, history = run_stage(
            model, ema, train_loader2, val_loader, device,
            epochs=args.s2_epochs, lr=args.s2_lr,
            warmup=args.s2_warmup, ema_decay=args.s2_ema,
            ckpt_dir=ckpt_dir, stage_name="stage2",
            history=history,
        )
        final_ckpt = s2_best
    else:
